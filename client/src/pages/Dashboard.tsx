...
function Dashboard() {
  ...
  const tasks = filter === "all" ? items : items.filter(t => t.status === filter)

  useEffect(() => {
    dispatch(fetchTasks())
  }, [])

  function alltask() { setFilter("all") }
  function todoTask() { setFilter("to do") }
  function inProgress() { setFilter("in progress") }
  function doneTask() { setFilter("done") }
